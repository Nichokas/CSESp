...
async fn main() {
    ...
    if cli.calificar {
        ...
        sleep(Duration::from_millis(500)).await;
        handle.text(r#"{"command": "message","identifier": "{\"channel\":\"SubmitCliChannel\"}","data": "{\"message\":\"velocidad\"}"}"#).unwrap();
        ...
        let mins = "Las letras siguen manteniendose como antes".to_string();
        let mays = "Se agregan los ... en cada espacio".to_string();
        ...
        // se mantine prueba
        {
            let resl:String = python_driver(&py_v,&p_name,b"aBcDeFgHiJkLmNopqrsTUvwxZY\n");
            if resl.contains("aBcDeFgHiJkLmNopqrsTUvwxZY") {
                println!("{}", (mins + bien).green());
                correct += 1
            } else {
                println!("{}", (mins + mal).red());
                println!("Se esperaba que la salida fuera {} y fue {}", "aBcDeFgHiJkLmNopqrsTUvwxZY".green(),resl.red())
            }
        }

        // ... prueba
        {
            let resl:String = python_driver(&py_v,&p_name,b"hola mundo\n");
            if resl.contains("hola...mundo") {
                println!("{}", (mays+bien).green());
                correct += 1;
            } else {
                println!("{}", (mays+mal).red());
                println!("Se esperaba que la salida fuera {} y fue {}", "hola...mundo".green(),resl.red())
            }
        }

        println!("==============================");
        println!("Pruebas correctas: {}/2",correct)
    }
}
